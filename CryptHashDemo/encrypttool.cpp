...
// 初始化AES 密钥，密钥用于加密解密
VOID EncryptTool::InitializePrivateKey(DWORD KeySize,UCHAR *KeyBytes)
{
    if (m_lpAes)
    {
        delete m_lpAes;
...
// 用AES加密算法加密数据
DWORD EncryptTool::OnAesEncrypt(LPVOID InBuffer,DWORD InLength,LPVOID OutBuffer)
{
    DWORD OutLength=0;
...
// 用AES加密算法解密数据
DWORD EncryptTool::OnAesUncrypt(LPVOID InBuffer,DWORD InLength,LPVOID OutBuffer)
{
    DWORD OutLength=0;
    if (m_lpAes==NULL||OutBuffer==NULL)
    {
